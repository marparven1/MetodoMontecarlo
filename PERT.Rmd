...
- El proyecto comienza en el instante \( 0 \). 
- La tarea \( j \)-ésima comienza en el instante \( s_{j} \), dura \( t_{j} \) instantes de tiempo y termina en el instante \( e_{j} = s_{j} + t_{j} \). 
- Cualquier tarea \( j \) **sin dependencias** comienza en \( s_{j} = 0 \). 
- El instante de comienzo de una tarea **con dependencias** es el máximo de los instantes en los que terminan estas últimas. 
  - Por ejemplo, \( s_{4} = e_{2} \), ya que la tarea 4 comienza en el instante en que acaba la tarea 2 
  - Otro ejemplo: \( s_{9} = \max(e_{5}, e_{6}, e_{7}) \). El proyecto al completo termina en el instante \( e_{10} \).
...
Queremos estimar la esperanza del instante en que termina la última tarea (10), suponiento que el tiempo que dura cada tarea es: \( t_{j} \sim \mathrm{Exp}(\lambda_{j}) \)
...
genera_duraciones_tareas <- function(duraciones_medias) {
  ...
  for (j in seq_len(10)) {
    lambda <- 1 / duraciones_medias[j] # lambda = 1/ media
    duraciones_tareas[j] <- rexp(1, rate = 1 / lambda) # Generación de números pseudoaleatorios exponenciales
  }
  duraciones_tareas
}
...
calcula_duracion_proyecto <- function(t) { # totalmente específico de este proyecto, para otro grafo tengo que usar otra función
  ...
  e_7 <- e_3 + t[7]
  e_8 <- e_3 + t[8]
  e_9 <- max(e_5, e_6, e_7) + t[9] # como el 9 depende de las tareas, 5,6,7, comienzo cuando acabe la última
...
```{r estimacion-1}
estimacion_duracion_proyecto <- mean(duraciones_proyecto) # estimación
...
## Variante con método montecarlo
...
alfa <- 1 - probabilidad_cobertura
percentil <- qnorm(1 - alfa / 2)
n_0 <- n_1 <- percentil^2 / 2 # n0 = n1 = 4.4
...
Por tanto, la probabilidad de que la duración del proyecto sea mayor de 40 es ínfima.
